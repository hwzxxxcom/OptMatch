...
def split_nx(G: nx.Graph, min_n = 30, max_n = 50, num = 1000):
    i = 0
    outputs = []
    while i < num:
        start = random.randint(0, len(G.nodes))
        sample_count = random.randint(min_n, max_n + 1)
        selected_nodes = [start]
        tag = True
        while True:
            cands = sum((list(G.neighbors(selected_node)) for selected_node in selected_nodes), start=[])
            cands = list(set(cands) - set(selected_nodes))
            if not cands and len(selected_nodes) < sample_count: 
                tag = False
                break
            selected_nodes.append(random.choice(cands))
            if len(selected_nodes) == sample_count: 
                break
        if tag:
            outputs.append(nx2nx(G.subgraph(selected_nodes)))
            i += 1
    return outputs
