...
def loaddataset(name: str, use_valedges_as_input: bool, load=None):
    if name in ["Cora", "Citeseer", "Pubmed"]:
        ...
        data.num_nodes = data.x.shape[0]
    elif name in ['yeast', 'wordnet', 'dblp']:
        meta_dict = {'dir_path': 'dataset/%s'%name,
            'eval metric': 'hits@20', 
            'download_name': name,
            ...
            'is hetero': False,
            'binary': False}
        dataset = PygLinkPropPredDataset(name=name, meta_dict=meta_dict)
