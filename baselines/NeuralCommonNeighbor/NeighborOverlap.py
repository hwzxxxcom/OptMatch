...
def train(model,
    ...
    for perm in PermIterator(
        ...
        if maskinput:
            ...
            adj = data.adj_t
        h = model(torch.ones_like(data.x), adj)
                ...
                if True:
                    for key, result in results.items():
                        ...
                        if valid_hits > bestscore[key][1]:
                            ...
                            if args.savemod:
                                torch.save(model.state_dict(), f"gmodel/{args.dataset}_model.pt")
                                torch.save(predictor.state_dict(), f"gmodel/{args.dataset}_predictor.pt")
                        print(key)
