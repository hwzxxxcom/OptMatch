...
def load_dataset(name):
    ...
    if name == "enzymes":
        ...
        dataset = Planetoid(root="../TrackCountPredict/nxdata", name="citeseer")
    elif name == "proteins":
        ...
        dataset = [g for g in nx.graph_atlas_g()[1:] if nx.is_connected(g) and len(g.nodes)]
    elif name in ['yeast', 'wordnet', 'dblp', 'youtube', 'wiki', 'google']:
        dataset = utils.split_nx(utils.load_grf_nx('./data/%s.graph' % name))
    if task == "graph":
...
class DiskDataSource(DataSource):
    ...
    def gen_batch(self, a, b, c, train, max_size=6, min_size=5, seed=None,
