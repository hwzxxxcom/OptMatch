...
if __name__ == "__main__":
    ...
    meta_dict = {'dir_path': 'dataset/%s'%args.dataname,
                'eval metric': 'hits@20', 
                'download_name': args.dataname,
                ...
                'is hetero': False,
                'binary': False}
    dataset = PygLinkPropPredDataset(name=args.dataname, root='./dataset',meta_dict=meta_dict,
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            print(epoch)
            loss = train(model, predictor, torch.ones([data.num_nodes, args.hidden_channels]), split_edge,
                        optimizer, args.batch_size, A, data, args, epoch)
            torch.save(model.state_dict(), 'saved_models/%s_model.pth' % args.dataname)
            torch.save(predictor.state_dict(), 'saved_models/%s_predictor.pth' % args.dataname)
